...
#   ./mix.sh            يستخدم ffmpeg من النظام
#   FFMPEG=/path/ffmpeg ./mix.sh
set -euo pipefail
cd "$(dirname "$0")"
FF="${FFMPEG:-ffmpeg}"

python3 sound.py jahiz-ad.wav          # الموسيقى
python3 voice.py vo-track.wav          # التعليق الصوتي في مواضعه
...
"$FF" -y -hide_banner -loglevel error -i jahiz-ad-silent.mp4 -i mix.wav \
  -c:v copy -c:a aac -b:a 192k -shortest -movflags +faststart jahiz-ad.mp4
...
echo "تم: jahiz-ad.mp4 (صوت + موسيقى) و jahiz-ad-music-only.mp4"
